...
// Mood to music mapping (alle jetzt als Arrays!)
const moodsToSongs = {
    ...
    messages: [
        {
            ...
            - romantic: Verträumte, liebevolle Techno-Stücke
            - angry: Aggressivere, druckvolle Techno-Beats`
        }
    ]
};
...
function playMoodMusic(mood) {
    let songList = moodsToSongs[mood];

    if (!songList || songList.length === 0) {
        console.warn(`⚠️ Keine Musikdateien für Mood "${mood}" gefunden.`);
        return;
    }
    ...
    console.log(`🎵 Spielt Mood "${mood}" Track: ${song}`);
    ...
    if (song) {
        ...
        console.warn(`⚠️ Song für Mood "${mood}" ist leer oder ungültig.`);
    }
}
